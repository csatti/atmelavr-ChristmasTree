...
#define INT_RED		_intensity >> 2
#define INT_GREEN	_intensity >> 1
#define INT_BLUE	_intensity >> 4

volatile uint16_t progcnt = 0;
volatile uint8_t prog[200];

volatile uint32_t timeReference = 0;	// time elapsed since start up (approx in 2 milliseconds)
...
void set_intensity(uint8_t intensity)
{
	...
	if (_color == 1) OCR0A = INT_RED;
	if (_color == 2) OCR2B = INT_GREEN;
	if (_color == 3) OCR0B = INT_BLUE;
}
...
void setup_timers(void)
{
	...
	TCNT2 = 0x1F; // offset compared to timer 0
	OCR2B = 0x7F;
	OCR2A = 0x7F;
	TIMSK2 = _BV(OCIE2B);
